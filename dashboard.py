...
# -----------------------------------------------------------------------------
    ...
    if df_trades.empty:
        ...
        styled_df = df_closed[display_cols].style.applymap(color_pnl, subset=['pnl'])\
            .format({
                'entry_price': lambda x: f"{x:.4f}" if pd.notnull(x) else "",
                'exit_price': lambda x: f"{x:.4f}" if pd.notnull(x) else "",
                'size': lambda x: f"${x:,.0f}" if pd.notnull(x) else "",
                'pnl': lambda x: f"${x:,.2f}" if pd.notnull(x) else "",
                'entry_time': lambda x: x.strftime("%Y-%m-%d %H:%M") if pd.notnull(x) else "",
                'exit_time': lambda x: x.strftime("%Y-%m-%d %H:%M") if pd.notnull(x) else ""
            })
...
# -----------------------------------------------------------------------------
    ...
    if not os.path.exists(DB_PATH):
        ...
        if df_sig.empty:
            ...
            # Drop duplicates in case DB has old duplicate entries
            df_sig = df_sig.drop_duplicates(subset=['symbol', 'timeframe', 'timestamp'], keep='first')
