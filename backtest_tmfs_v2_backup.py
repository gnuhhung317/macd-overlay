...
def calculate_indicators(df, config: BacktestConfig):
    ...
    st1, _ = calculate_supertrend(df, 0.618, config.atr_period)
    st2, _ = calculate_supertrend(df, 1.618, config.atr_period)
    st3, _ = calculate_supertrend(df, 2.618, config.atr_period)
    
    df['avg_st'] = (st1 + st2 + st3) / 3
    df['smoothed_st'] = df['avg_st'].ewm(span=config.super_length, adjust=False).mean()
    
    df['di_plus'], df['di_minus'] = calculate_dmi(df, config.dmi_length)
    df['atr'] = calculate_atr(df, 13)
    df['atr_swing'] = calculate_atr(df, config.swing_length)
    
    def is_pivot_high(x):
        mid = config.swing_length
        if x[mid] == np.max(x):
            if np.sum(x == x[mid]) == 1:
                return x[mid]
        return np.nan
    ...
    df['pivot_high'] = df['high'].rolling(window=config.swing_length*2+1).apply(is_pivot_high, raw=True)
    df['pivot_low'] = df['low'].rolling(window=config.swing_length*2+1).apply(is_pivot_low, raw=True)
    
    return df

def calculate_signals(df, config: BacktestConfig):
    print("Calculating trendlines and breakout states...")
    ...
    upper_breakout_state = np.zeros(len(df))
    lower_breakout_state = np.zeros(len(df))
    
    curr_upper_state = 0
    curr_lower_state = 0
    
    # We iterate manually for the state machine of breakout states
    for i in range(config.swing_length * 2 + 1, len(df)):
        pivot_h_val = df['pivot_high'].iloc[i]
        pivot_l_val = df['pivot_low'].iloc[i]
        
        ph_hit = not np.isnan(pivot_h_val)
        pl_hit = not np.isnan(pivot_l_val)
        
        if ph_hit:
            upper_slope[i] = df['atr_swing'].iloc[i] / config.swing_length * config.atr_mult
            upper_trendline[i] = pivot_h_val - (upper_slope[i] * config.swing_length)
            curr_upper_state = 0
        else:
            upper_slope[i] = upper_slope[i-1]
            upper_trendline[i] = upper_trendline[i-1] - upper_slope[i]
            if df['close'].iloc[i] > upper_trendline[i]:
                curr_upper_state = 1
        
        if pl_hit:
            lower_slope[i] = df['atr_swing'].iloc[i] / config.swing_length * config.atr_mult
            lower_trendline[i] = pivot_l_val + (lower_slope[i] * config.swing_length)
            curr_lower_state = 0
        else:
            lower_slope[i] = lower_slope[i-1]
            lower_trendline[i] = lower_trendline[i-1] + lower_slope[i]
            if df['close'].iloc[i] < lower_trendline[i]:
                curr_lower_state = 1
        
        upper_breakout_state[i] = curr_upper_state
        lower_breakout_state[i] = curr_lower_state
        
    df['upper_trendline'] = upper_trendline
    df['lower_trendline'] = lower_trendline
    df['upper_breakout_state'] = upper_breakout_state
    df['lower_breakout_state'] = lower_breakout_state
    
    # Trigger signals
    df['long_trigger'] = (df['upper_breakout_state'] == 1) & (df['upper_breakout_state'].shift(1) == 0) & \
                        (df['di_plus'] > df['di_minus']) & (df['close'] > df['smoothed_st'])
    df['short_trigger'] = (df['lower_breakout_state'] == 1) & (df['lower_breakout_state'].shift(1) == 0) & \
                         (df['di_minus'] > df['di_plus']) & (df['close'] < df['smoothed_st'])
    
    # Exit signals (crossunder/crossover)
    df['exit_signal_long'] = (df['close'].shift(1) < df['smoothed_st'].shift(1)) & (df['close'].shift(2) >= df['smoothed_st'].shift(2))
    df['exit_signal_short'] = (df['close'].shift(1) > df['smoothed_st'].shift(1)) & (df['close'].shift(2) <= df['smoothed_st'].shift(2))
