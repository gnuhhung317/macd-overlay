...
class MEXCFetcher:
    def __init__(self):
        ...
        # MEXC history for swaps generally starts around 2021
        self.start_ts = int(datetime(2021, 1, 1, tzinfo=timezone.utc).timestamp() * 1000)
        self.data_dir = Path("mexc-data")
    ...
    def get_all_symbols(self):
        """Get all USDT-M swap symbols from MEXC"""
    ...
    def fetch_funding_rates(self, symbol):
        """MEXC funding rate fetch workaround"""
        try:
    ...
    def run(self, limit_coins=None):
        ...
        for i, symbol in enumerate(symbols, 1):
            ...
            print(f"[{i}/{len(symbols)}] MEXC: {symbol} -> {clean_name}...")
